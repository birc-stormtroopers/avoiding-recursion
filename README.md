...
**Exercise:** Traversing using a `parent` pointer can get a lot harder if the trees are not binary, since then you need to work out which child to traverse next when you return searching up through `parents`, but there are various ways to do it. Can you find a way?
